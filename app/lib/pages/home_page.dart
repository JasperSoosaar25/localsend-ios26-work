...
enum HomeTab {
  receive(Icons.wifi, 'arrow.down.circle', 'arrow.down.circle.fill'),
  send(Icons.send, 'paperplane', 'paperplane.fill'),
  settings(Icons.settings, 'gearshape', 'gearshape.fill')
  ;

  const HomeTab(this.icon, this.iosSymbol, this.iosSelectedSymbol);

  final IconData icon;
  final String iosSymbol;
  final String iosSelectedSymbol;
    ...
    return DropTarget(
      ...
      child: ResponsiveBuilder(
        builder: (sizingInformation) {
          final nativeIOS = sizingInformation.isMobile && supportsNativeIOS26Controls;
          return Scaffold(
            extendBody: nativeIOS,
            body: Row(
              children: [
            ...
            bottomNavigationBar: sizingInformation.isMobile
                ? nativeIOS
                      ? IOS26NativeTabBar(
                          ...
                          onSelected: (index) => vm.changeTab(HomeTab.values[index]),
                          items: HomeTab.values
                              .map(
                                (tab) => IOS26TabItem(
                                  label: tab.label,
                                  symbol: tab.iosSymbol,
                                  selectedSymbol: tab.iosSelectedSymbol,
                                ),
                              )
