...
double _hashProgress(
  SendSessionState sendState,
  FileTransferNotifier transferNotifier,
) {
  final files = sendState.files.values;
  final totalBytes = files.fold<int>(0, (prev, curr) => prev + curr.file.size);
  if (totalBytes == 0) {
    return sendState.files.isEmpty
        ? 0
        : sendState.hashedFileCount / sendState.files.length;
  }
                              ...
                              child: TextButton.icon(
                                onPressed: !targetDevice.https
                                    ...
                                    : () async => await context.push(
                                        () => VerifyPage(
                                          fingerprint: CombinedFingerprint.load(
                                            context,
                          ...
                          children: [
                            switch (sendState.status) {
                              SessionStatus.waiting => Padding(
                                ...
                                child:
                                    sendState.hashedFileCount <
                                        sendState.files.length
                                    ? Column(
                                        children: [
                                          Text(
                                            t.sendPage.calculatingChecksum(
                                              curr: sendState.hashedFileCount,
                                              n: sendState.files.length,
                                            ),
                                          ...
                                          SizedBox(
                                            ...
                                            child: LinearProgressIndicator(
                                              value: _hashProgress(
                                                sendState,
                                                ref.watch(fileTransferProvider),
                                              ),
                                            ),
                                          ),
                                        ],
                            ...
                            Center(
                              child: supportsNativeIOS26Controls
                                  ? SizedBox(
                                      width: 220,
                                      height: 54,
                                      child: IOS26NativeButton(
                                        title: waiting
                                            ? t.general.cancel
                                            : t.general.close,
                                        symbol: waiting ? 'xmark' : 'checkmark',
